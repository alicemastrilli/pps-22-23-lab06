...
import scala.reflect.Manifest.{Any, Nothing, Null}
...
trait NotTwoConsecutive[T] extends Parser[T]:
  private[this] var last = Option.empty[T]

  abstract override def parse(t: T): Boolean =
    if last.isEmpty || last.get != t then {last = Option(t); super.parse(t); } else false
